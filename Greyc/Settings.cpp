...
namespace
{
	const int InterpolationToScript[] =
	{
	...
	int ScriptToInterpolation(int iVal)
	{
		for(int i = 0; InterpolationToScript[i] != -1; ++i)
			if(InterpolationToScript[i] == iVal)
				return i;
		return 0;
	}
	...
	AlgorithmOptions::DisplayMode ScriptToDisplayMode(int iVal)
	{
		for(int i = 0; DisplayModeToScript[i] != -1; ++i)
			if(DisplayModeToScript[i] == iVal)
				return (AlgorithmOptions::DisplayMode) i;
		return (AlgorithmOptions::DisplayMode) 0;
	}
}
...
static void ReadFromKeys(GetKeys &keys, Parameters &params, bool bReadOptions)
{
	...
	while(keys.GetNextKey(type))
	{
		...
		{
		case keyInitialGaussian:params.FilterSettings.m_fPreBlur = keys.GetFloat(); break;
		case keyAmplitude:	params.FilterSettings.amplitude = keys.GetFloat(); break;
		case keySharpness:	params.FilterSettings.sharpness = keys.GetPercent(); break;
		case keyAnisotropy:	params.FilterSettings.anisotropy = keys.GetPercent(); break;
		case keyAlpha:		params.FilterSettings.alpha = keys.GetPercent(); break;
		case keySigma:		params.FilterSettings.sigma = keys.GetPercent(); break;
		case keyGfact:		params.FilterSettings.gfact = keys.GetPercent(); break;
		case keyNormalize:	params.FilterSettings.gfact = keys.GetPercent() * -1; break;
		case keyDl:		params.FilterSettings.dl = keys.GetPercent(); break;
		case keyDa:		params.FilterSettings.da = keys.GetFloat(); break;
		case keyGaussPrec:	params.FilterSettings.gauss_prec = keys.GetPercent(); break;
		case keyFastApprox:	params.FilterSettings.fast_approx = keys.GetBoolean(); break;
		case keyAltAmplitude:	params.FilterSettings.alt_amplitude = keys.GetBoolean(); break;
		case keyIterations:	params.FilterSettings.iterations = keys.GetInteger(); break;
		case keyThreads:	params.FilterOptions.nb_threads = keys.GetInteger(); break;
		case keyGPU:		params.FilterOptions.m_bGPU = keys.GetBoolean(); break;
		case keyInterpolation:
		{
			DescriptorEnumID e = keys.GetEnum();
			params.FilterSettings.interpolation = ScriptToInterpolation(e);
			break;
		}
		case keyDisplayMode:
		{
...
#define TO_SAVE(n) (!bOmitDefaults || params.FilterSettings.n != DefaultSettings.n)
	if(TO_SAVE(m_fPreBlur))		keys.PutFloat(keyInitialGaussian, params.FilterSettings.m_fPreBlur, unitPixels);
	if(TO_SAVE(amplitude))		keys.PutFloat(keyAmplitude, params.FilterSettings.amplitude, unitPixels);
	if(TO_SAVE(sharpness))		keys.PutPercent(keySharpness, params.FilterSettings.sharpness);
	if(TO_SAVE(anisotropy))		keys.PutPercent(keyAnisotropy, params.FilterSettings.anisotropy);
	if(TO_SAVE(alpha))		keys.PutPercent(keyAlpha, params.FilterSettings.alpha);
	if(TO_SAVE(sigma))		keys.PutPercent(keySigma, params.FilterSettings.sigma);

	if(TO_SAVE(gfact))
	{
		if(params.FilterSettings.gfact > 0)
			keys.PutPercent(keyGfact, params.FilterSettings.gfact);
		else
			keys.PutPercent(keyGfact, -params.FilterSettings.gfact);
	}
	
	if(TO_SAVE(dl))			keys.PutPercent(keyDl, params.FilterSettings.dl);
	if(TO_SAVE(da))			keys.PutFloat(keyDa, params.FilterSettings.da, unitAngle);
	if(TO_SAVE(gauss_prec))		keys.PutPercent(keyGaussPrec, params.FilterSettings.gauss_prec);
	if(TO_SAVE(fast_approx))	keys.PutBoolean(keyFastApprox, params.FilterSettings.fast_approx);
	/*if(TO_SAVE(alt_amplitude))*/	keys.PutBoolean(keyAltAmplitude, params.FilterSettings.alt_amplitude);
	if(TO_SAVE(iterations))		keys.PutInteger(keyIterations, params.FilterSettings.iterations);
	if(TO_SAVE(interpolation))	keys.PutEnum(keyInterpolation, InterpolationToScript[params.FilterSettings.interpolation], typeInterpolation);

	if(bWriteOptions)
	{
		keys.PutInteger(keyThreads, params.FilterOptions.nb_threads);
		keys.PutBoolean(keyGPU, params.FilterOptions.m_bGPU);
...
void ReadScriptParameters(Parameters &params)
{
	...
	keys.Rewind();
	params.FilterSettings = AlgorithmSettings();
